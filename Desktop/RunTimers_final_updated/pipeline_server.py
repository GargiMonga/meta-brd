"""
pipeline_server.py  ── RunTimers Compliance Monitor
────────────────────────────────────────────────────
No API key required. PDF ingestion uses pdfplumber + local NLP.
...
# ── PDF pipeline: always available, no key needed ────────────────────────────
try:
...
# These never need an API key anymore
pipeline       = CompliancePipeline()       if _pipeline_available else None
explainer      = ViolationExplainer()       if _pipeline_available else None
...
def health():
    return {
        "status":          "ok",
        "service":         "compliance-pipeline",
        "version":         "2.0.0",
        "pdf_pipeline":    _pipeline_available,
        # api_key is no longer required — always True
        "llm_available":   False,
        "free_mode":       True,
...
async def ingest_pdf(file: UploadFile = File(...)):
    ...
    Rules are extracted locally — no API key required.
    """
    if not _pipeline_available:
        raise HTTPException(
            status_code=503,
            detail=f"pdfplumber not installed. Run: pip install pdfplumber  [{_pipeline_import_error}]"
        )
...
async def compare_pdfs(file_a: UploadFile = File(...), file_b: UploadFile = File(...)):
    """Compare two PDF policies and find contradictions — no API key required."""
    if not _pipeline_available:
        raise HTTPException(status_code=503, detail="PDF pipeline unavailable — install pdfplumber")
    try:
        import tempfile
        bytes_a, bytes_b = await file_a.read(), await file_b.read()
        with tempfile.NamedTemporaryFile(suffix=".pdf", delete=False) as ta:
            ta.write(bytes_a); path_a = ta.name
        with tempfile.NamedTemporaryFile(suffix=".pdf", delete=False) as tb:
            tb.write(bytes_b); path_b = tb.name
        try:
...
def run_scan(req: ScanRequest = ScanRequest()):
    ...
    for v in raw_violations:
        ...
        if req.include_explanations and explainer:
...
def explain_violation(req: ViolationExplainRequest):
    if not explainer:
        raise HTTPException(status_code=503, detail="Explainer unavailable — install pdfplumber")
...
def openenv_records():
    return {"records": db.get_all_records(), "count": len(db.get_all_records())}


@app.get("/openenv/rules")
def openenv_rules():
    return {"rules": db.get_rules(), "count": len(db.get_rules())}


if __name__ == "__main__":
    port = int(os.environ.get("PORT", 7861))
    uvicorn.run("pipeline_server:app", host="0.0.0.0", port=port, reload=False)